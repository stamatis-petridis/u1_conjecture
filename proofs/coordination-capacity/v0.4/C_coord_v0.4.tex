\subsection{The Fundamental Law of Persistence}
...
$\kappa_S$ quantifies how much \emph{anchored history} the system
...
\mathcal{P}_S(\tau)
=
\liminf_{t \to \infty} \kappa_S(t, \tau)
=
\liminf_{t \to \infty}
\frac{\mathcal{S}_S(t)}{\mathcal{H}_S(t, \tau)}.
\]
When the limit exists, we simply write
$\mathcal{P}_S(\tau) = \lim_{t \to \infty} \kappa_S(t, \tau)$.
\end{definition}
...
\begin{enumerate}
  ...
  \item \textbf{Gradient ascent on $\kappa$.}
    ...
    on $\kappa_S$ over evolutionary time.
  ...
  \item \textbf{Singularity regime.}
    ...
    \quad
    \frac{d}{dt}\kappa_{S_\star}(t,\tau) > 0
    \quad
    \text{for all sufficiently large } t,
    \]
    then $\kappa_{S_\star}(t,\tau) \to \infty$ and the subsystem
...
dominate. The universe selects for high $\kappa$: structures that
...
$\mathcal{H} \to 0$. In this regime, $\kappa$ is unbounded and the
...
viewed as candidates for high $\kappa$ structures:
...
\begin{itemize}
  ...
  \item Biological lineages expend energy to reduce uncertainty about
        ...
        thereby increasing $\kappa$ generation by generation.
